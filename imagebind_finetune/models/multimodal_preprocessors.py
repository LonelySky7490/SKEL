...
class PatchEmbedGeneric(nn.Module):
    ...
    def forward(self, x): # vision: [b*10, 3, 224, 224]
        x = self.proj(x) # vision: [b*10, 1280, 1, 16, 16]
        # B C (T) H W -> B (T)HW C
        x = x.flatten(2).transpose(1, 2) # vision: [bs*10, 256, 1280]
...
class RGBDTPreprocessor(VerboseNNModule):
    ...
    def tokenize_input_and_cls_pos(self, input, stem, mask):
        ...
        tokens = stem(input) # vision: [bs*10, 256, 1280]
        ...
        if self.use_pos_embed:
            pos_embed = self.pos_embedding_helper.get_pos_embedding(input, tokens) # [bs*10, 257, 1280]
            tokens = tokens + pos_embed
        if self.use_type_embed:
            tokens = tokens + self.type_embed.expand(B, -1, -1)

        return tokens # vision: [bs*10, 257, 1280]
