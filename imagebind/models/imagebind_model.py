    ...
    def forward(self, inputs):
        ...
        for modality_key, modality_value in inputs.items():
            ...
            if modality_value is not None:
                ...
                modality_value = self.modality_trunks[modality_key](**trunk_inputs) #! Transformer encoders runs here
                # |_-> [bs*46(train_close/total_class), 77, 1024] for text (77),
                # |_-> [bs*10, 229, 768] for audio patches (1+228),
                # |_-> [bs*10, 257, 1280] for visual patches (1+256),
                modality_value = self.modality_heads[modality_key](
                    modality_value, **head_inputs
                )
                # |_-> [bs*46, 1024] for whole text
                # |_-> [bs*10, 1024] for audio [cls] token
                # |_-> [bs*10, 1024] for visual [cls] token
